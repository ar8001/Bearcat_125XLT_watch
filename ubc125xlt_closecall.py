...
MODE_LABELS = {
    0: "SCAN (Close Call off)",
    1: "SEARCH (Close Call Priority)",
    2: "CC DND",
    3: "CLOSE CALL ONLY",
}
...
def apply_close_call_settings(ser, band_state, cc_mode):
    ...
    if resp != "PRG,OK":
        raise ScannerError("Could not enter programming mode (%r)" % resp)
    ...
    if resp != "CLC,OK":
        ...
        raise ScannerError("Could not set operating mode (%r)" % resp)

    resp = send_command(ser, "EPG")
    if resp != "EPG,OK":
        raise ScannerError("Could not exit programming mode (%r)" % resp)
...
def parse_glg(response):
    ...
    (freq_mhz, mod, kanal_nr, kanal_name) zurück, oder
    (None, None, None, None) wenn aktuell kein Signal anliegt."""
    ...
    if not (20.0 <= freq_mhz <= 1400.0):
        ...
        if 20.0 <= alt <= 1400.0:
            ...
            return None, None, None, None
    # Kanalname (Feld 7) und Kanal-/Speichernummer (Feld 11, Wertebereich
    # 1-500 laut CIN-Befehl) - beide nur bei Scan/Search meist gefüllt, bei
    # reinem Close Call üblicherweise leer - laut Reverse Engineering.
    chan_name = parts[7].strip() if len(parts) > 7 else ""
    chan_num = parts[11].strip() if len(parts) > 11 else ""
    return freq_mhz, mod, chan_num, chan_name


def derive_bank_mem(chan_num):
    """Leitet Speicherbank (1-10) und Speicherplatz innerhalb der Bank
    (1-50) aus der absoluten Kanalnummer (1-500) ab, wie sie vom CIN-
    Befehl adressiert wird (10 Bänke x 50 Kanäle). Liefert ("", "") wenn
    keine gültige Kanalnummer vorliegt (z. B. reiner Close-Call-Treffer
    ohne gespeicherten Kanal)."""
    if not chan_num or not chan_num.strip().isdigit():
        return "", ""
    idx = int(chan_num.strip())
    if not (1 <= idx <= 500):
        return "", chan_num.strip()
    bank = ((idx - 1) // 50) + 1
    mem = ((idx - 1) % 50) + 1
    return str(bank), str(mem)
...
class App:
    ...
    def poll_signal(self):
        ...
        try:
            ...
            return
        freq, mod, chan_num, chan_name = parse_glg(resp)
        ...
        if now - self.pending_since >= CONFIRM_SECONDS:
            ...
            self._confirm_hit(freq, mod, self.pending_chan, self.pending_name, now)
    ...
    def _confirm_hit(self, freq, mod, chan_num, chan_name, now_ts):
        ...
        stat["mod"] = mod
        if chan_num:
            stat["chan_num"] = chan_num
            stat["bank"], stat["mem"] = derive_bank_mem(chan_num)
        if chan_name:
            stat["name"] = chan_name
    ...
    def _write_log_snapshot(self):
        ...
        writer.writerow([
            "Date", "First_Seen", "Last_Seen", "Frequency_MHz", "Modulation",
            "Bank", "Memory", "Name", "Count", "Duration_Last_s", "Duration_All_s",
        ])
    ...
    def draw(self):
        ...
        col = 0
        col = self._put(1, col, "Close Call Bands: ", self.CYAN, w)
        ...
        header_line = "%-8s  %10s  %-4s  %-3s  %-3s  %-10s  %-8s  %-7s  %-4s" % (
            "Last", "Freq(MHz)", "Mod", "BNK", "MEM", "Name", "Dur.Last", "Dur.All", "Cnt",
        )
        ...
        for i, stat in enumerate(stats[:visible_rows]):
            ...
            if is_active:
                live_elapsed = now_ts - self.active_start
                dur_last = live_elapsed
                dur_all = stat["total_duration"] + live_elapsed
            else:
                dur_last = stat["duration"]
                dur_all = stat["total_duration"]
            line = "%-8s  %10.4f  %-4s  %-3s  %-3s  %-10.10s  %-8s  %-7s  x%-3d" % (
                stat["last_seen"].strftime("%H:%M:%S"),
                stat["freq"],
                stat["mod"],
                stat["bank"] or "-",
                stat["mem"] or "-",
                stat["name"] or "",
                self._format_duration(dur_last),
                self._format_duration(dur_all),
                stat["count"],
            )
        ...
        help_line = (
            "[L] Log  [1-5] Band  [C] CC  [S] Search  [A] Scan  [Q] Quit"
        )
        if self.log_active and self.log_path:
            help_line += "   File: %s" % os.path.basename(self.log_path)
